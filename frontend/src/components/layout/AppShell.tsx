...
import { registerHandDraftSyncTriggers } from "@/features/hands/lib/draftSync";
...
export function AppShell() {
  ...
  useEffect(() => {
    if (!user) return;
    return registerHandDraftSyncTriggers();
  }, [user]);
  const hideBottomNav = Boolean(
    ...
    matchPath({ path: "/hand/:slug", end: true }, location.pathname) ||
    matchPath({ path: "/hand/new", end: true }, location.pathname) ||
    matchPath({ path: "/hand/draft/:draftId", end: true }, location.pathname) ||
    matchPath({ path: "/hand/:slug/edit", end: true }, location.pathname) ||
  ...
  const hidePushBanner =
    ...
    location.pathname === "/search" ||
    location.pathname === "/notifications" ||
    location.pathname.startsWith("/hand/");
